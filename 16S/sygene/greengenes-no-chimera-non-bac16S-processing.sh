#cd fastq-join
echo 拼接
join_paired_ends.py -f /media/biolinux/LENOVO_imcdul/16s/$1 -r /media/biolinux/LENOVO_imcdul/16s/$2 -o temp
echo 质控
split_libraries_fastq.py -q 19 --barcode_type not-barcoded --store_demultiplexed_fastq  -o temp --sample_ids $1 -i temp/fastqjoin.join.fastq
echo  格式转换
sed 's/ .*/;/g;s/>.*/&&/g;s/;>/;barcodelabel=/g;s/_[0-9]*;$/;/g' temp/seqs.fna > temp/seqs_usearch.fa
echo 序列去冗余
/home/biolinux/16s-sygene/usearch10 -fastx_uniques temp/seqs_usearch.fa -fastaout temp/seqs_unique.fa -minuniquesize 2 -sizeout
echo 聚类OTU
/home/biolinux/16s-sygene/usearch10 -cluster_otus temp/seqs_unique.fa -otus temp/otus.fa -uparseout temp/uparse.txt -relabel Otu
echo 查看OTU数量
grep '>' -c temp/otus.fa
# 下载Usearch8推荐的参考数据库RDP
#wget http://drive5.com/uchime/rdp_gold.fa
echo 基于RDP数据库比对去除已知序列的嵌合体\(可选\)
/home/biolinux/16s-sygene/usearch10 -uchime2_ref temp/otus.fa \
    -db /home/biolinux/16s-sygene/gg_13_8_otus/rep_set/97_otus.fasta \
...
echo 生成OTU表
/home/biolinux/16s-sygene/usearch10 -usearch_global temp/seqs_usearch.fa -db temp/otus_non_chimera.fa -otutabout temp/otu_table.txt -biomout temp/otu_table.biom -strand plus -id 0.97 -threads 10
...
echo 物种注释
assign_taxonomy.py -i temp/otus_non_chimera.fa \
